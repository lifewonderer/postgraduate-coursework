...
data_refs = np.loadtxt('collect_bp2refs.out')
data_refb = np.loadtxt('collect_bp2refb.out')
data_avxs = np.loadtxt('collect_bp2avxs.out')
data_avxb = np.loadtxt('collect_bp2avxb.out')
...
for i in range(12):
    ...
    plt.title('BP2 Problem size = %i' %n[0], fontsize=20)
    ...
    plt.savefig('bp2_time_size%i.png' %n[0])
    plt.close()
    '''
    ...
    plt.grid()
    plt.xticks(np.arange(1, 17))
    plt.savefig('bp2_dofs_size%i.png' %n[0])
    plt.close()
...
for i in range(16):
    ...
    plt.title('BP2 Polynomial degree = %i' %p[0], fontsize=20)
    ...
    plt.legend(fontsize=14)
    plt.grid()
    plt.savefig('bp2_time_degree%i.png' %p[0])
    plt.close()
    '''
    ...
    plt.legend(fontsize=14)
    plt.grid()
    plt.savefig('bp2_dofs_degree%i.png' %p[0])
    plt.close()